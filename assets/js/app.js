/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `ISC2CC` namespace with
...
(function () {
  ...
  var HIGH = (S.exam && S.exam.scaleHigh) || 1000;
  var MINS = (S.exam && S.exam.minutes) || 120;
  var MAXQ = (S.exam && S.exam.maxQuestions) || 100;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("ISC2CC." + k, fb); },
      set: function (k, v) { write("ISC2CC." + k, v); },
      del: function (k) { try { localStorage.removeItem("ISC2CC." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the ISC2 CC exam is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the ISC2 CC exam &mdash; the high-stakes details every candidate should internalize before sitting the test.",
      foot: MINS + " min · " + MAXQ + " items · " + PASS + " to pass", cta: "Read",
      onClick: function () { openExamMechanics(); }
    }));
    g1.appendChild(card({
      icon: "🧭", tag: "CAREER", title: "Career Guidance", tagCls: "ghost",
      desc: "Where ISC2 Certified in Cybersecurity sits on the career ladder: the entry-level governance, operations, and analyst roles it unlocks for newcomers to the field.",
      foot: "Foundational · DoD 8140/8570", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every ISC2 CC acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    var b5 = dashSection("08", "Mock certification exam", MAXQ + " questions · timed");
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the ISC2 CC exam &mdash; 100 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic 120-minute simulation: 100 questions weighted to the exact domain percentages, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard.",
      foot: "100 questions · 120 minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official ISC2 Certified in Cybersecurity exam outline, the free ISC2 self-paced training, and trusted community study guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.ISC2CC_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. Access Control" });
  ...
  function computeReadiness() {
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? Math.round(bestMock / HIGH * 100) : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "/" + HIGH) : "not attempted" },
  ...
  function readinessOutcome(R) {
    if (R.bestMock != null) {
      var m = R.bestMock;
      if (m >= PASS + 50) return { cls: "good", text: "Strong pass projected. Your best mock of " + m + " out of " + HIGH + " clears the " + PASS + " passing line with margin." };
      if (m >= PASS) return { cls: "good", text: "On track to pass. Your best mock of " + m + " out of " + HIGH + " is at or above the " + PASS + " passing line. Keep it consistent." };
      if (m >= PASS - 70) return { cls: "warn", text: "Borderline. Your best mock of " + m + " out of " + HIGH + " sits just under the " + PASS + " line. A focused push should get you there." };
      return { cls: "crit", text: "Not yet. Your best mock of " + m + " out of " + HIGH + " is below passing. Keep building before you book the exam." };
    }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates ISC2CC.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.ISC2CC_VER ? "?v=" + window.ISC2CC_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.open(readerView("Domain " + id, d.title, "ISC2 CC Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: ISC2 CC", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where ISC2 CC fits in your path", S.careerGuidance || [])); }
      ...
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var scoreCell = r.mode === "mock"
            ? el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.scaled + " / " + HIGH })
